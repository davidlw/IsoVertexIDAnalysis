...
void visualization(int eventNumber, std::string fileName, std::string tag = "", float maxZ = 10){
  ...
  topDummy->GetYaxis()->SetRangeUser(-0.5,maxZ);
  ...
  for(unsigned int i = 0; i<z.size(); i++){
    ...
    for(unsigned int j = 0; j<nTracks[index]; j++){
      ...
      float xStop = 0;
      float yStop = 3;
      if( (theta<TMath::Pi()/2.0) && (TMath::Tan(theta) < maxZ/(maxZ-zTrk[index][j])) ){
        xStop = maxZ;
        yStop = TMath::Tan(theta) * (maxZ-zTrk[index][j]);
      }
      else if( theta<TMath::Pi()/2.0 ){
        xStop = zTrk[index][j] + 1.0/TMath::Tan(theta)*(maxZ);
        yStop = maxZ;
      } 
      else if( (theta>TMath::Pi()/2.0) && (TMath::Tan(TMath::Pi()-theta) < maxZ/(TMath::Abs(zTrk[index][j] + maxZ))) ){
        xStop = -maxZ;
        yStop = TMath::Tan(TMath::Pi()-theta) * (TMath::Abs(zTrk[index][j] + maxZ));
      }
      else if( theta>TMath::Pi()/2.0 ){
        xStop = zTrk[index][j] - 1.0/TMath::Tan(TMath::Pi()-theta)*(maxZ) ;
        yStop = maxZ;
      }
      else{
  ...
  for(unsigned int i = 0; i<z.size(); i++){
    ...
    mR[i]->SetMarkerSize(1.4);
    mR[i]->Draw("same");
    if(contaminationFracByWeight[index] < 0.0001){
      std::cout << "hi" << std::endl;
      mR_noContam[i] = new TMarker(zVtx[index], 0, 8);
      mR_noContam[i]->SetMarkerColor(kGreen+1);
      mR_noContam[i]->SetMarkerSize(1.1);
